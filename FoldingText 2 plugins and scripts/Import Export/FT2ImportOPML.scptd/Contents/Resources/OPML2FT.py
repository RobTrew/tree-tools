...
def ft_doc(lst, int_headings, int_remaining):
	...
	for peer in lst:
		...
		if (bln_header):
			str_text = ''.join([str_text, '#' * int_level ,' ', str_text])
		else:
			str_text = ''.join([str_text, '\t' * int_level ,'- ', str_text])

		# Append any @done tags
		if ('done' in lst_keys):
			str_text = ''.join([str_text, ' @done'])
		...
		if ('note' in lst_keys):
			for str_line in dct_props['note'].split('\n'):
				str_text = ''.join([str_text, str_note_indent, str_line, '\n'])

		# Recurse with any children
		if (len(lst_chiln) > 0):
			str_text = ''.join([str_text, ft_doc(lst_chiln, int_headings, int_remaining - 1)])
	return str_text
